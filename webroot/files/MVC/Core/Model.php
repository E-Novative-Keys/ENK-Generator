	...
	*/
	public function find($request = array(), $fetch = PDO::FETCH_OBJ)
	{
		...
		if(isset($request['conditions']))
		{
			...
			else
			{
				...
				foreach($request['conditions'] as $key => $value)
				{
					...
					if(!is_numeric($value) && !strstr($key, 'IN'))
		...
		if(isset($request['like']))
			$sql .= (isset($request['conditions']) ? ' AND ' : ' WHERE ').'`'.$request['like'][0].'` LIKE "'.$request['like'][1].'"';
	...
	*/
	public function save($data)
	{
		...
		foreach($data as $k => $v)
		{
			...
			// Update ou Insert
			if($primaryFound)
				$sql = 'UPDATE '.(($this->table != false) ? strtolower($this->table) : strtolower($k).'s').' SET '.implode(',', $fields).' WHERE `'.$primary.'`=:'.$primary;
			else
				$sql = 'INSERT INTO '.(($this->table != false) ? strtolower($this->table) : strtolower($k).'s').' SET '.implode(',', $fields);
